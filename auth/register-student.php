...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!verifyCSRFToken($_POST['csrf_token'] ?? '')) {
        ...
        $validator->required('student_id', $student_id)->maxLength('student_id', $student_id, 20);
        $validator->required('department_id', $department_id)->numeric('department_id', $department_id);
        $validator->required('enrollment_year', $enrollment_year)->numeric('enrollment_year', $enrollment_year);
        $validator->required('semester', $semester)->numeric('semester', $semester);
        ...
        if ($validator->hasErrors()) {
            ...
            $result = $auth->register($email, $password, 'student', $extraData);
                    ...
                    <form method="POST" action="">
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="number" class="form-control" id="enrollment_year" name="enrollment_year" min="2000" max="<?= date('Y') + 1 ?>" required>
                            </div>
                            <div class="col-md-6 mb-3">
